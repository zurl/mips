 ...
 */
import { buildAsmInstructionRRR, buildAsmInstructionRRN, buildAsmInstructionRN, buildAsmInstructionN, buildAsmInstructionR, buildAsmInstructionRR } from "./instruction";
...
export class MIPSAssembler {
    ...
    __buildIns(arr) {
        if (arr[3]) {
            if (arr[4]) {
                if (arr[4].charAt(0) == '$') {
                    return buildAsmInstructionRRR(arr[1], arr[2], arr[3], arr[4]);
                }
                else if (!isNaN(parseInt(arr[4]))) {
                    return buildAsmInstructionRRN(arr[1], arr[2], arr[3], parseInt(arr[4]));
                }
                else {
                    return buildAsmInstructionRRN(arr[1], arr[3], arr[2], this.__hashString(arr[4]));
                }
            }
            else if (arr[5]) {
                return buildAsmInstructionRRN(arr[1], arr[2], arr[5], parseInt(arr[3]));
            }
            else if (arr[3].charAt(0) == '$') {
                return buildAsmInstructionRR(arr[1], arr[2], arr[3]);
            }
            else if (!isNaN(parseInt(arr[3]))) {
                return buildAsmInstructionRN(arr[1], arr[2], parseInt(arr[3]));
            }
            else {
                return buildAsmInstructionRN(arr[1], arr[2], this.__hashString(arr[3]));
            }
        }
        else if (arr[2].charAt(0) == '$') {
            return buildAsmInstructionR(arr[1], arr[2]);
        }
        else {
            return buildAsmInstructionN(arr[1], (this.__hashString(arr[2], false)) & 0x03FFFFFF);
        }
    }
    parse(source) {
